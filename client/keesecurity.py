...
class DatabaseViewer(tk.Frame):
    ...
    def __init__(self, master, accounts, password):
        ...
        if accounts:
            self.accounts = [[str(z) for z in x] for x in accounts]
        else:
    ...
    def table_click(self, event):
        ...
        if region == "separator":
            ...
            if self.column_selected != 3:
                self.sortcol = {0: 0,
                                1: 1,
                                2: 2,
                                4: 3}[self.column_selected]
            self.update_table()
    ...
    def get_selected_values(self, column="all"):
        ...
        row = self.accounts[self.index_mapping[self.row_selected - self.start_index]]
        # Return entire row
        if column == "all":
            return row
    ...
    def add_entry(self):
        ...
        def add():
            service = str(service_entry.get())
            login = str(login_entry.get())
            notes = str(notes_entry.get(1.0, "end").replace("\n", " "))
            self.accounts.append([service, login, notes])
            self.index_mapping.append(len(self.index_mapping))
    ...
    def delete_entry(self):
        ...
        del self.accounts[self.index_mapping[self.row_selected - self.start_index]]
        # reset index mapping
        self.index_mapping = [x for x in range(len(self.accounts))]
    ...
    def edit_entry(self):
        ...
        accounts_index = {1: 0,
                          2: 1,
                          4: 2}[self.column_selected]
        self.get_selected_values("all")[accounts_index] = \
            str(self.edit_textbox.get(1.0, "end")).replace("\n", " ")
        self.update_table()
    ...
    def update_table(self):
        ...
        if len(self.accounts) != 0:
            if self.sortcol == 0:
                ...
                self.index_mapping = list(self.index_mapping)

            # Add entries
            for index, row in enumerate(sorted_accounts):
